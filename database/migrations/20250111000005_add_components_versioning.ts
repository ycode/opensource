...
/**
 * Migration: Add versioning to components
 * 
 * Adds is_published and publish_key columns to support draft/published workflow.
 * Allows dual-record pattern like pages where draft and published versions coexist.
 */
...
export async function up(knex: Knex): Promise<void> {
  ...
  // Format: name can be the same, but is_published must differ
...
export async function down(knex: Knex): Promise<void> {
  ...
  await knex.schema.alterTable('components', (table) => {
    // Drop indexes
    table.dropIndex('publish_key', 'idx_components_publish_key');
    table.dropIndex('is_published', 'idx_components_is_published');
