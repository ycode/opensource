...
export function extractComponentTranslatableItems(
  ...
  if (layers && Array.isArray(layers) && layers.length > 0) {
    extractComponentLayerTextItems(layers, component.id, items);
  }
...
/**
 * Recursively extract text items from layers for components
 */
function extractComponentLayerTextItems(
  layers: Layer[],
  componentId: string,
  items: TranslatableItem[]
): void {
  for (const layer of layers) {
    // Extract text from this layer
    const text = extractLayerText(layer);
    if (text) {
      items.push({
        key: `component:${componentId}:layer:${layer.id}:text`,
    ...
    // Recursively process children
    if (layer.children && Array.isArray(layer.children) && layer.children.length > 0) {
      extractComponentLayerTextItems(layer.children, componentId, items);
    }
  }
}
  ...
  for (const field of fields) {
    ...
    items.push({
      ...
      info: {
        ...
        label: field.name,
        description: isSlugField ? 'Will affect URLs generated by dynamic pages' : undefined,
      },
    });
  }
